...
%% HW 4: PCA-Based Face Recognition
...
tbl2 = readtable(sTestPath);

testPaths = tbl2.Path;
testWidths = tbl2.Width;
testHeights = tbl2.Height;
testRoiX1 = tbl2.Roi_X1;
testRoiY1 = tbl2.Roi_Y1;
testRoiX2 = tbl2.Roi_X2;
testRoiY2 = tbl2.Roi_Y2;
testClasses = tbl2.ClassId;
% 
...
% eigensigns together represent 39% of the variance in the data. The first
% eigensign represents 22% of the variance alone. With 40 eigensigns, we can 
% represent 99.6% of the total variance. This means we can get a
...
% arranged in the rough shape of a sign. The dark areas appear around the
% eyes and the lighter areas around the forehead of the first two
% eigensigns. The third eigensigns has a much lighter area around the jaw
% than the first two. The last eigensign has much more defined hair, eye,
% nose, ear and mouth regions. If we consider any image as a combination of these
...
recon10 = reshape(pca_reconstruction(chosen_sign, eigsigns, 10)+mean(A, 1), M, N);
recon20 = reshape(pca_reconstruction(chosen_sign, eigsigns, 20)+mean(A, 1), M, N);
recon30 = reshape(pca_reconstruction(chosen_sign, eigsigns, 30)+mean(A, 1), M, N);
recon40 = reshape(pca_reconstruction(chosen_sign, eigsigns, 40)+mean(A, 1), M, N);
...
figure;
subplot(4, 3, 1), pcolor(flipud(chosen_sign)), shading interp, colormap(gray), title(strcat('original sign: s', num2str(A_labels(r))))
subplot(4, 3, 4), pcolor(flipud(chosen_sign)), shading interp, colormap(gray), title(strcat('original sign: s', num2str(A_labels(r))))
subplot(4, 3, 7), pcolor(flipud(chosen_sign)), shading interp, colormap(gray), title(strcat('original sign: s', num2str(A_labels(r))))
subplot(4, 3, 10), pcolor(flipud(chosen_sign)), shading interp, colormap(gray), title(strcat('original sign: s', num2str(A_labels(r))))
